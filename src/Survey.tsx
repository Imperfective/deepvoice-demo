...
const AGE_OPTS = ['10대', '20대', '30대', '40대', '50대', '60대 이상'];
const GENDER_OPTS = ['남성', '여성', '응답 안 함'];
const EDU_OPTS = ['고졸 이하', '대학 재학·졸업', '대학원 이상'];
const USAGE_OPTS = ['1시간 미만', '1~3시간', '3~5시간', '5시간 이상'];
...
function QLabel({ text, required = true, hint }: { text: string; required?: boolean; hint?: string }) {
  return (
...
function Likert({ value, onChange, lowLabel = '전혀 아니다', highLabel = '매우 그렇다' }: {
...
function Choice({ options, value, onChange }: { options: string[]; value: string | null; onChange: (v: string) => void }) {
  return (
...
function MultiChoice({ options, values, onToggle }: { options: string[]; values: string[]; onToggle: (v: string) => void }) {
  return (
    ...
    return (
      <div style={{ minHeight: '100vh', background: DS.callBg, display: 'flex', flexDirection: 'column', fontFamily: 'var(--font-body)' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 24px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 21, fontWeight: 800, color: DS.ink, letterSpacing: '-0.4px' }}>설문이 제출됐습니다</div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: DS.callBg, display: 'flex', flexDirection: 'column', fontFamily: 'var(--font-body)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div>
          <QLabel text="④ 방해도 — 이 경고가 통화에 지나치게 방해되었습니까?" hint="※ 역척도: 높을수록 방해가 컸음을 의미합니다" />
        ...
        <div>
          ...
          <TextArea value={improve} onChange={setImprove} placeholder="예: 경고가 더 크면 좋겠다 / 소리가 갑작스러웠다 등" />
        </div>
        <div>
